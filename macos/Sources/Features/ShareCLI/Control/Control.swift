...
/// One point-in-time surface inventory returned by `surface.list`.
///
/// `complete` is an integrity assertion, not a convenience flag. Consumers
/// may reconcile missing surfaces as exits only when it is true. A provider
/// that is starting, resynchronizing, or unavailable must return an empty
/// incomplete inventory instead of an array that could be mistaken for proof
/// that no panes exist. `generation` is a provider-owned decimal string for a
/// strictly monotonic stable inventory marker. It is retained while an
/// inventory is incomplete, so clients can identify the last complete snapshot
/// without treating it as current exit evidence.
public struct SurfaceInventory: Codable, Equatable, Sendable {
    public let surfaces: [SurfaceRecord]
    public let complete: Bool
    public let generation: String?

    public init(surfaces: [SurfaceRecord], complete: Bool, generation: String? = nil) {
        self.surfaces = surfaces
        self.complete = complete
        self.generation = generation
    }

    public static let unavailable = SurfaceInventory(surfaces: [], complete: false)
}
...
public protocol SurfaceProvider: Sendable {
    func surfaceInventory() async throws -> SurfaceInventory
...
public struct ControlDispatcher: Sendable {
    ...
    private func dispatch(method: String, params: [String: Any]) async throws -> Any {
        switch method {
        case "surface.list":
            return try jsonObject(await provider.surfaceInventory())
        ...
        case "surface.io.subscribe":
            ...
            let subscriptionID = try await liveEvents.createSubscription(
                surfaceID: surfaceID,
